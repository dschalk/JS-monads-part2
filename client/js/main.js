...
function view(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14, m15, m16, m17, m18, m19, mI1, mI2) { 
    ...
    [ h('h1', {style: {textAlign: 'center', color: '#ffba66'}}, 'JS-monads-part2'),
      ...
      cow.monad,
      h('p', 'Click the next button to execute the indicated code, then click the mMZ2.release() button beneath it four times.'  ),
      ...
      h('p', 'Next, a demonstration of monads handling websockets messages. This is the foundation for the game demonstrated in page 3 of this series. In order to create a unique socket, please enter some name.'  ),
...
function updateR(event) {
  ...
  .bnd(mM6.ret).bnd(mM7.ret).bnd(mM8.ret).bnd(mM9.ret).bnd(mM10.ret).bnd(mM11.ret).bnd(mM12.ret).bnd(mM13.ret).bnd(mM15.ret).bnd(mM16.ret).bnd(mM17.ret).bnd(mM18.ret).bnd(mM19.ret).bnd(mMZ1.ret).bnd(mMZ2.ret).bnd(() => mM1.ret([])).bnd(() => mM14.ret('Score: ' + mM13.x));
  oldVnode = patch(oldVnode, newVnode());
}
